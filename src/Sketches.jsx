...
const Shape = props => { // Path and Circle manager
  const [len, setLength] = useState(1)
  const { path, circle, on, w=200, h=200 } = props
...
const particleSVG = on => <svg width={200} height={200} viewBox="0 0 261 288" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const lightningSVG = on => <svg width={200} height={200} viewBox="0 0 427 321" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const sketchData = [
  { name: 'Particle Mesh', icon: particleSVG, settings: {} },
  { name: 'Lightning', icon: lightningSVG, settings: {} }
]


const Sketches = props => {
  const [curSketch, setSketch] = useState(sketchData[0])

  return (
    sketchData.map(sketch => {
      const cur = sketch.name === curSketch.name
      return <div onClick={_ => setSketch(sketch)} style={{ margin: 30, border: '1px solid #0000' }}>{sketch.icon(cur)}</div>
    })
  )
}
